...
export default function Dashboard() {
  ...
  const timeDisplay = mounted ? (
    <>
      <div suppressHydrationWarning className="text-3xl font-mono text-cyan-400 mb-1">
  ...
  // Modify the performance chart to only render on client
  const performanceChart = mounted ? <PerformanceChart /> : null
                        ...
                        <div className="h-64 w-full relative bg-slate-800/30 rounded-lg border border-slate-700/50 overflow-hidden">
                          ...
                          {mounted && (
                            <div className="absolute bottom-4 right-4 bg-slate-900/80 backdrop-blur-sm rounded-md px-3 py-2 border border-slate-700/50">
                              ...
                              <div suppressHydrationWarning className="text-lg font-mono text-cyan-400">{cpuUsage}%</div>
                            </div>
                          )}
                        </div>
                      </TabsContent>
...
function PerformanceChart() {
  const [chartData, setChartData] = useState<{ cpu: number; mem: number; net: number }[]>([])
